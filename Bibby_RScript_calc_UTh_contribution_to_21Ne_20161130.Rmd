...
geochem_data <- read.csv("~/Box\ Sync/Bibby_UCSB_box/Research/Antarctica/Ong_Valley_Moraine_Canyon/U-Th_in_Ne-21/data_original/data_dirty/GeochemistryDataAntML2015.csv" , stringsAsFactors = FALSE) %>%
as_data_frame() %>%
...
### Plot the expected concentration of U and Th isotopes (iso_ppm)
```{r}
extended_geochem_data %>%
...
This is just doing some datamanagement, to get everything into a common data frame so that we can start doing math across entries. It's ungle, but it makes life a little easier.
...
parent_atoms = A(numeric_ppm/1E6)*B(abundance)*C(wtkg*1000)*D(avogadro/mass)
A: 1ppm = 1g/1E6g, so whatever we measured as ppm is the same as that concentration in grams over 1,000,000 grams. This gives us grams of element in 1E6 grams of solution
B: What fraction of the U is actually U238? Multiply by abundance
C: convert mass of solution from to kg to g for unit similarity. Then multiply mass of solution in grams by concentration. This gives us total grams of the element before solution.
D: now we have grams of the element but we want atoms, so multiply by conversion factor (6.022E23 atoms=1 molecular mass of element [238g for U])
...
extended_geochem_data=extended_geochem_data %>% 
  ...
  mutate(parent_atoms = ((numeric_ppm/1E6)*(abundance)*(wtkg*1000)*(avogadro/mass))) %>%
  as_data_frame()
...
extended_geochem_data %>%
  ggplot(aes(x=sample_id,y=parent_atoms, fill=isotope))+
  geom_bar(stat = "identity")+
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
...
```
Looks like the number of parent atoms matches the intial element concentration. As expected

Lets calculate again but use just iso_ppm and don't multiply by abundance
###Start Here
```{r}
extended_geochem_data %>% 
  mutate(parent_atoms2 = ((iso_ppm/1E6)*(wtkg*1000)*(avogadro/mass))) %>%
  mutate(difference = parent_atoms2-parent_atoms) %>% 
  as_data_frame() %>% 
  select(sample_id, isotope, numeric_ppm, iso_ppm, parent_atoms,parent_atoms2, difference) %>% 
  datatable()
```

####Alpha decays
How many radioactive alpha decays have occurred during the lifetime of the sample? This is based on the age of the sample, the original concentration of parent isotopes, and the 1/2 life of the isotope. The important thing here is to decide what the sample age is first. Currently it's set at `r sample_age` yrs. Eventually we'll either need to calculate an expected inital age for every sample or use a single number. But currently I'm just using a reasonable max for `r site_location`.

Formula: alpha_decays = parent_atoms*(exp(lambda*sample_age)-1)
```{r}
extended_geochem_data=extended_geochem_data %>% 
  mutate(alpha_decays = parent_atoms*(exp(lambda*sample_age)-1))
