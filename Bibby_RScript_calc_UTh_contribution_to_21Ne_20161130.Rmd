...
(looking at the difference column comparing "parent"" to "parent2")
...
extended_geochem_data %>%
  ...
  select(sample_id,current_atoms,initial_atoms) %>% 
  datatable()
...
Formula: alpha_decays = current_atoms*(exp(lambda*sample_age)-1)
```{r}
extended_geochem_data=extended_geochem_data %>% 
