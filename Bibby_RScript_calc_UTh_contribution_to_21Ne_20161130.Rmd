...
my_data <- read.csv("~/Box Sync/Research/!active_projects/U-Th_in_Ne-21/data_original/GeochemistryDataAntML2015.csv" , stringsAsFactors = FALSE) %>%
as_data_frame() %>%
rename(sample_id = ANALYTE, wtkg = WtKg)
datatable(my_data)
```

###Read noble gas data csv from Dan Morgan

```{r}
#Open noble gas results from Dan
noble_gas_data <- read.csv("~/Box Sync/Research/!active_projects/U-Th_in_Ne-21/data_cleaned_b4_import/Ne21_OV_MC_Aug2015_final_clean.csv" , stringsAsFactors = FALSE) %>%
as_data_frame()
datatable(noble_gas_data)


```

Clean up the geochemistry datatable. (remove special characters)
...
### Compare the noble gas measurement to in-situ
