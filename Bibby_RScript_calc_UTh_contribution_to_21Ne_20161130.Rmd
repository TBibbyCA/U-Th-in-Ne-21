...
```{r}
my_data <- tbl_df(read.csv("~/Box Sync/Research/U-Th_in_Ne-21/GeochemistryDataAntML2015.csv" , stringsAsFactors = FALSE)) %>% 
  rename(sample_id = ANALYTE, wtkg = WtKg)
my_data
```
##Remove special characters from data and adjust headers
```{r}
...
true_data
```
##Create a dataframe of constants:
```{r}
#Analytical Resuts
sampleAge = 100e6 #Ma
...
##Calculate alpha decays given age of sample
```{r}
extended_elements=extended_elements %>% 
...
##Calculate 21Ne atoms produced from isotope decays due to age of sample
