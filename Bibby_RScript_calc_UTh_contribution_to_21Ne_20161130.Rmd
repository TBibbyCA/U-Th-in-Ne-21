...
# take geochem_data, call it true_data
true_data <- geochem_data %>% 
  filter(!(sample_id %in% header_names)) %>% 
  as_data_frame() %>% 
...
total_data <- lapply(elements, atom_splitter, dat = true_data, elem_info = elem_info) %>% 
  bind_rows()
...
### Merge geochem_data and elem_info into a single dataframe called: total_data
...
total_data=mutate(total_data, avogadro,sampleAge)
